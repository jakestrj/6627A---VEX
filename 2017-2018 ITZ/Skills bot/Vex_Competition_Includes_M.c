...
static void displayStatusAndTime( bool reset )
{
  ...
  displayLCDPos(0, 0);
	if (bIfiRobotDisabled){
		displayNextLCDString("GyroReady: ");
		displayNextLCDNumber((int)gyroReady);
	}

  displayLCDPos(1, 0);
	if (bIfiRobotDisabled)
	  displayNextLCDString("AUTO_N: ");
	else
	{
	  if (bIfiAutonomousMode)
	...
	displayNextLCDNumber(autonomousMode);
	/*displayNextLCDNumber(nDisplayAndStatusTimer / 600, 2);1
	displayNextLCDChar(':');
